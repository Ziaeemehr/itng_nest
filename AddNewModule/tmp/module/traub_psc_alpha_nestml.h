...
*
*  2020-03-26 06:36:32.642856
*/
#ifndef TRAUB_PSC_ALPHA_NESTML
#define TRAUB_PSC_ALPHA_NESTML
 ...
 */
extern "C" inline int traub_psc_alpha_nestml_dynamics( double, const double y[], double f[], void* pnode );
...
*/
class traub_psc_alpha_nestml : public nest::Archiving_Node{
public:
  /**
  * The constructor is only used to create the model prototype in the model manager.
  */
  traub_psc_alpha_nestml();
  ...
  */
  traub_psc_alpha_nestml(const traub_psc_alpha_nestml &);
...
private:
  ...
  friend class nest::RecordablesMap<traub_psc_alpha_nestml>;
  friend class nest::UniversalDataLogger<traub_psc_alpha_nestml>;
  ...
  */
  ...
  /**
    ...
    */
  struct Buffers_ {
    Buffers_(traub_psc_alpha_nestml &);
    Buffers_(const Buffers_ &, traub_psc_alpha_nestml &);

    /** Logger for all analog data */
    nest::UniversalDataLogger<traub_psc_alpha_nestml> logger_;
  ...
  inline double get_alpha_n_init() const {
    return 0.032 * (S_.ode_state[State_::V_m] / 1.0 + 52.0) / (1.0 - std::exp((-(S_.ode_state[State_::V_m] / 1.0 + 52.0)) / 5.0));
  }

  inline double get_beta_n_init() const {
    return 0.5 * std::exp((-(S_.ode_state[State_::V_m] / 1.0 + 57.0)) / 40.0);
  }

  inline double get_alpha_m_init() const {
    return 0.32 * (S_.ode_state[State_::V_m] / 1.0 + 54.0) / (1.0 - std::exp((-(S_.ode_state[State_::V_m] / 1.0 + 54.0)) / 4.0));
  }

  inline double get_beta_m_init() const {
    return 0.28 * (S_.ode_state[State_::V_m] / 1.0 + 27.0) / (std::exp((S_.ode_state[State_::V_m] / 1.0 + 27.0) / 5.0) - 1.0);
  }

  inline double get_alpha_h_init() const {
    return 0.128 * std::exp((-(S_.ode_state[State_::V_m] / 1.0 + 50.0)) / 18.0);
  }

  inline double get_beta_h_init() const {
    return 4.0 / (1.0 + std::exp((-(S_.ode_state[State_::V_m] / 1.0 + 27.0)) / 5.0));
  }
  ...
  inline double get_I_syn_in__d() const {
    return S_.ode_state[State_::I_syn_in__d];
  }
  inline void set_I_syn_in__d(const double __v) {
    S_.ode_state[State_::I_syn_in__d] = __v;
  }
  ...
  inline double get_I_syn_ex__d() const {
    return S_.ode_state[State_::I_syn_ex__d];
  }
  inline void set_I_syn_ex__d(const double __v) {
    S_.ode_state[State_::I_syn_ex__d] = __v;
  }
  ...
  */
  ...
  static nest::RecordablesMap<traub_psc_alpha_nestml> recordablesMap_;

  friend int traub_psc_alpha_nestml_dynamics( double, const double y[], double f[], void* pnode );
  
/** @} */
}; /* neuron traub_psc_alpha_nestml */

inline nest::port traub_psc_alpha_nestml::send_test_event(
...
inline nest::port traub_psc_alpha_nestml::handles_test_event(nest::SpikeEvent&, nest::port receptor_type){
...
inline nest::port traub_psc_alpha_nestml::handles_test_event(
...
inline void traub_psc_alpha_nestml::get_status(DictionaryDatum &__d) const{  
  ...
  def<double>(__d, "I_syn_in", get_I_syn_in());
      
  def<double>(__d, "I_syn_in__d", get_I_syn_in__d());
      
  def<double>(__d, "I_syn_ex", get_I_syn_ex());
      
  def<double>(__d, "I_syn_ex__d", get_I_syn_ex__d());
...
inline void traub_psc_alpha_nestml::set_status(const DictionaryDatum &__d){
...
// ignores 'beta_h_init' double' since it is an function and setter isn't defined
  ...
  double tmp_I_syn_in = get_I_syn_in();
  updateValue<double>(__d, "I_syn_in", tmp_I_syn_in);

  

  double tmp_I_syn_in__d = get_I_syn_in__d();
  updateValue<double>(__d, "I_syn_in__d", tmp_I_syn_in__d);

  

  double tmp_I_syn_ex = get_I_syn_ex();
  updateValue<double>(__d, "I_syn_ex", tmp_I_syn_ex);

  

  double tmp_I_syn_ex__d = get_I_syn_ex__d();
  updateValue<double>(__d, "I_syn_ex__d", tmp_I_syn_ex__d);
  ...
  set_I_syn_in(tmp_I_syn_in);



  set_I_syn_in__d(tmp_I_syn_in__d);



  set_I_syn_ex(tmp_I_syn_ex);



  set_I_syn_ex__d(tmp_I_syn_ex__d);
...
#endif /* #ifndef TRAUB_PSC_ALPHA_NESTML */
#endif /* HAVE GSL */