...
* Recordables map
* ---------------------------------------------------------------- */
nest::RecordablesMap<wb_receptors_nestml> wb_receptors_nestml::recordablesMap_;

namespace nest
{
  // Override the create() method with one call to RecordablesMap::insert_()
  // for each quantity to be recorded.
  template <> void RecordablesMap<wb_receptors_nestml>::create(){
  // use standard names whereever you can for consistency!  

  insert_("alpha_n_init", &wb_receptors_nestml::get_alpha_n_init);

  insert_("beta_n_init", &wb_receptors_nestml::get_beta_n_init);

  insert_("alpha_m_init", &wb_receptors_nestml::get_alpha_m_init);

  insert_("beta_m_init", &wb_receptors_nestml::get_beta_m_init);

  insert_("alpha_h_init", &wb_receptors_nestml::get_alpha_h_init);

  insert_("beta_h_init", &wb_receptors_nestml::get_beta_h_init);

  insert_("Inact_h", &wb_receptors_nestml::get_Inact_h);

  insert_("Act_n", &wb_receptors_nestml::get_Act_n);

  insert_("V_m", &wb_receptors_nestml::get_V_m);

  insert_("g_AMPA__d", &wb_receptors_nestml::get_g_AMPA__d);

  insert_("g_AMPA", &wb_receptors_nestml::get_g_AMPA);

  insert_("g_NMDA__d", &wb_receptors_nestml::get_g_NMDA__d);

  insert_("g_NMDA", &wb_receptors_nestml::get_g_NMDA);

  insert_("g_GABAA__d", &wb_receptors_nestml::get_g_GABAA__d);

  insert_("g_GABAA", &wb_receptors_nestml::get_g_GABAA);

  insert_("g_GABAB__d", &wb_receptors_nestml::get_g_GABAB__d);

  insert_("g_GABAB", &wb_receptors_nestml::get_g_GABAB);

  insert_("I_syn_ampa", &wb_receptors_nestml::get_I_syn_ampa);

  insert_("I_syn_nmda", &wb_receptors_nestml::get_I_syn_nmda);

  insert_("I_syn_gaba_a", &wb_receptors_nestml::get_I_syn_gaba_a);

  insert_("I_syn_gaba_b", &wb_receptors_nestml::get_I_syn_gaba_b);

  insert_("I_syn", &wb_receptors_nestml::get_I_syn);
  }
}
 ...
 * is a part of the wb_receptors_nestml's constructor.
 * ---------------------------------------------------------------- */
wb_receptors_nestml::Parameters_::Parameters_(){}

wb_receptors_nestml::State_::State_(){}

/* ----------------------------------------------------------------
* Parameter and state extractions and manipulation functions
* ---------------------------------------------------------------- */

wb_receptors_nestml::Buffers_::Buffers_(wb_receptors_nestml &n):
  logger_(n), spike_inputs_( std::vector< nest::RingBuffer >( SUP_SPIKE_RECEPTOR - 1 ) ), __s( 0 ), __c( 0 ), __e( 0 ){
  // Initialization of the remaining members is deferred to
  // init_buffers_().
}

wb_receptors_nestml::Buffers_::Buffers_(const Buffers_ &, wb_receptors_nestml &n):
  logger_(n), spike_inputs_( std::vector< nest::RingBuffer >( SUP_SPIKE_RECEPTOR - 1 ) ), __s( 0 ), __c( 0 ), __e( 0 ){
  // Initialization of the remaining members is deferred to
  // init_buffers_().
}
 ...
 * ---------------------------------------------------------------- */
wb_receptors_nestml::wb_receptors_nestml():Archiving_Node(), P_(), S_(), B_(*this)
{
  ...
  S_.alpha_n_init = (-0.05) * (S_.ode_state[State_::V_m] / 1.0 + 34.0) / (std::exp((-0.1) * (S_.ode_state[State_::V_m] / 1.0 + 34.0)) - 1.0); // as real
  
  S_.beta_n_init = 0.625 * std::exp((-(S_.ode_state[State_::V_m] / 1.0 + 44.0)) / 80.0); // as real
  
  S_.alpha_m_init = 0.1 * (S_.ode_state[State_::V_m] / 1.0 + 35.0) / (1.0 - std::exp((-0.1*1.0) * (S_.ode_state[State_::V_m] / 1.0 + 35.0*1.0))); // as real
  
  S_.beta_m_init = 4.0 * std::exp((-(S_.ode_state[State_::V_m] / 1.0 + 60.0)) / 18.0); // as real
  
  S_.alpha_h_init = 0.35 * std::exp((-(S_.ode_state[State_::V_m] / 1.0 + 58.0)) / 20.0); // as real
  
  S_.beta_h_init = 5.0 / (std::exp((-0.1) * (S_.ode_state[State_::V_m] / 1.0 + 28.0)) + 1.0); // as real
...
wb_receptors_nestml::wb_receptors_nestml(const wb_receptors_nestml& __n):
...
wb_receptors_nestml::~wb_receptors_nestml(){ 
...
void wb_receptors_nestml::init_state_(const Node& proto){
  const wb_receptors_nestml& pr = downcast<wb_receptors_nestml>(proto);
  S_ = pr.S_;
}



extern "C" inline int wb_receptors_nestml_dynamics(double, const double ode_state[], double f[], void* pnode){
  typedef wb_receptors_nestml::State_ State_;
  // get access to node so we can almost work as in a member function
  assert( pnode );
  const wb_receptors_nestml& node = *( reinterpret_cast< wb_receptors_nestml* >( pnode ) );

  // ode_state[] here is---and must be---the state vector supplied by the integrator,
  // not the state vector in the node, node.S_.ode_state[].
  
  f[State_::Inact_h] = ((0.35 * std::exp(-((ode_state[State_::V_m] / 1.0 + 58.0)) / 20.0)) * (1 - ode_state[State_::Inact_h]) - (5.0 / (std::exp(-(0.1) * (ode_state[State_::V_m] / 1.0 + 28.0)) + 1.0)) * ode_state[State_::Inact_h]) / 1.0;
  f[State_::Act_n] = ((-(0.05) * (ode_state[State_::V_m] / 1.0 + 34.0) / (std::exp(-(0.1) * (ode_state[State_::V_m] / 1.0 + 34.0)) - 1.0)) * (1 - ode_state[State_::Act_n]) - (0.625 * std::exp(-((ode_state[State_::V_m] / 1.0 + 44.0)) / 80.0)) * ode_state[State_::Act_n]) / 1.0;
  f[State_::V_m] = (-(((node.get_g_Na() * ((0.1 * (ode_state[State_::V_m] / 1.0 + 35.0) / (1.0 - std::exp(-(0.1*1.0) * (ode_state[State_::V_m] / 1.0 + 35.0*1.0)))) / ((0.1 * (ode_state[State_::V_m] / 1.0 + 35.0) / (1.0 - std::exp(-(0.1*1.0) * (ode_state[State_::V_m] / 1.0 + 35.0*1.0)))) + (4.0 * std::exp(-((ode_state[State_::V_m] / 1.0 + 60.0)) / 18.0)))) * ((0.1 * (ode_state[State_::V_m] / 1.0 + 35.0) / (1.0 - std::exp(-(0.1*1.0) * (ode_state[State_::V_m] / 1.0 + 35.0*1.0)))) / ((0.1 * (ode_state[State_::V_m] / 1.0 + 35.0) / (1.0 - std::exp(-(0.1*1.0) * (ode_state[State_::V_m] / 1.0 + 35.0*1.0)))) + (4.0 * std::exp(-((ode_state[State_::V_m] / 1.0 + 60.0)) / 18.0)))) * ((0.1 * (ode_state[State_::V_m] / 1.0 + 35.0) / (1.0 - std::exp(-(0.1*1.0) * (ode_state[State_::V_m] / 1.0 + 35.0*1.0)))) / ((0.1 * (ode_state[State_::V_m] / 1.0 + 35.0) / (1.0 - std::exp(-(0.1*1.0) * (ode_state[State_::V_m] / 1.0 + 35.0*1.0)))) + (4.0 * std::exp(-((ode_state[State_::V_m] / 1.0 + 60.0)) / 18.0)))) * ode_state[State_::Inact_h] * (ode_state[State_::V_m] - node.get_E_Na())) + (node.get_g_K() * ode_state[State_::Act_n] * ode_state[State_::Act_n] * ode_state[State_::Act_n] * ode_state[State_::Act_n] * (ode_state[State_::V_m] - node.get_E_K())) + (node.get_g_L() * (ode_state[State_::V_m] - node.get_E_L())))) + node.get_I_e() + node.B_.I_stim_grid_sum_ + ((-(ode_state[State_::g_AMPA]) * (ode_state[State_::V_m] - node.get_AMPA_E_rev())) + (-(ode_state[State_::g_NMDA]) * (ode_state[State_::V_m] - node.get_NMDA_E_rev()) / (1 + std::exp((node.get_NMDA_Vact() - ode_state[State_::V_m]) / node.get_NMDA_Sact()))) + (-(ode_state[State_::g_GABAA]) * (ode_state[State_::V_m] - node.get_GABA_A_E_rev())) + (-(ode_state[State_::g_GABAB]) * (ode_state[State_::V_m] - node.get_GABA_B_E_rev())))) / node.get_C_m();
...
void wb_receptors_nestml::init_buffers_(){
  ...
  B_.__sys.function = wb_receptors_nestml_dynamics;
...
void wb_receptors_nestml::calibrate(){
  B_.logger_.init();
 ...
 *
 */
void wb_receptors_nestml::update(nest::Time const & origin,const long from, const long to){
  double __t = 0;
...
void wb_receptors_nestml::handle(nest::DataLoggingRequest& e){
  B_.logger_.handle(e);
}

//
double wb_receptors_nestml::compute_synapse_constant(double Tau_1, double Tau_2, double g_peak) const
...
void wb_receptors_nestml::handle(nest::SpikeEvent &e){
...
void wb_receptors_nestml::handle(nest::CurrentEvent& e){
  assert(e.get_delay_steps() > 0);
