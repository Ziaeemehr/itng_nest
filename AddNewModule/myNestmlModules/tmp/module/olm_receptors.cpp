...
#include "olm_receptors.h"


/* ----------------------------------------------------------------
* Recordables map
* ---------------------------------------------------------------- */
nest::RecordablesMap<olm_receptors> olm_receptors::recordablesMap_;

namespace nest
{
  // Override the create() method with one call to RecordablesMap::insert_()
  // for each quantity to be recorded.
  template <> void RecordablesMap<olm_receptors>::create(){
  // use standard names whereever you can for consistency!  

  insert_("alpha_n_init", &olm_receptors::get_alpha_n_init);

  insert_("beta_n_init", &olm_receptors::get_beta_n_init);

  insert_("alpha_m_init", &olm_receptors::get_alpha_m_init);

  insert_("beta_m_init", &olm_receptors::get_beta_m_init);

  insert_("alpha_h_init", &olm_receptors::get_alpha_h_init);

  insert_("beta_h_init", &olm_receptors::get_beta_h_init);

  insert_("r_h_inf_init", &olm_receptors::get_r_h_inf_init);

  insert_("Act_m", &olm_receptors::get_Act_m);

  insert_("Inact_h", &olm_receptors::get_Inact_h);

  insert_("Act_n", &olm_receptors::get_Act_n);

  insert_("Act_r_h", &olm_receptors::get_Act_r_h);

  insert_("V_m", &olm_receptors::get_V_m);

  insert_("g_AMPA__d", &olm_receptors::get_g_AMPA__d);

  insert_("g_AMPA", &olm_receptors::get_g_AMPA);

  insert_("g_NMDA__d", &olm_receptors::get_g_NMDA__d);

  insert_("g_NMDA", &olm_receptors::get_g_NMDA);

  insert_("g_GABAA__d", &olm_receptors::get_g_GABAA__d);

  insert_("g_GABAA", &olm_receptors::get_g_GABAA);

  insert_("g_GABAB__d", &olm_receptors::get_g_GABAB__d);

  insert_("g_GABAB", &olm_receptors::get_g_GABAB);

  insert_("I_syn", &olm_receptors::get_I_syn);
  }
}
 ...
 * is a part of the olm_receptors's constructor.
 * ---------------------------------------------------------------- */
olm_receptors::Parameters_::Parameters_(){}

olm_receptors::State_::State_(){}

/* ----------------------------------------------------------------
* Parameter and state extractions and manipulation functions
* ---------------------------------------------------------------- */

olm_receptors::Buffers_::Buffers_(olm_receptors &n):
  logger_(n), spike_inputs_( std::vector< nest::RingBuffer >( SUP_SPIKE_RECEPTOR - 1 ) ), __s( 0 ), __c( 0 ), __e( 0 ){
  // Initialization of the remaining members is deferred to
  // init_buffers_().
}

olm_receptors::Buffers_::Buffers_(const Buffers_ &, olm_receptors &n):
  logger_(n), spike_inputs_( std::vector< nest::RingBuffer >( SUP_SPIKE_RECEPTOR - 1 ) ), __s( 0 ), __c( 0 ), __e( 0 ){
  // Initialization of the remaining members is deferred to
  // init_buffers_().
}
 ...
 * ---------------------------------------------------------------- */
olm_receptors::olm_receptors():Archiving_Node(), P_(), S_(), B_(*this)
{
...
olm_receptors::olm_receptors(const olm_receptors& __n):
...
void olm_receptors::init_state_(const Node& proto){
  const olm_receptors& pr = downcast<olm_receptors>(proto);
  S_ = pr.S_;
}



extern "C" inline int olm_receptors_dynamics(double, const double ode_state[], double f[], void* pnode){
  typedef olm_receptors::State_ State_;
  // get access to node so we can almost work as in a member function
  assert( pnode );
  const olm_receptors& node = *( reinterpret_cast< olm_receptors* >( pnode ) );
...
void olm_receptors::init_buffers_(){
  ...
  if ( B_.__s == 0 ){
    B_.__s = gsl_odeiv_step_alloc( gsl_odeiv_step_rkf45, 13 );
  } else {
 ...
 *
 */
void olm_receptors::update(nest::Time const & origin,const long from, const long to){
  double __t = 0;
...
void olm_receptors::handle(nest::DataLoggingRequest& e){
  B_.logger_.handle(e);
}

//
double olm_receptors::compute_synapse_constant(double Tau_1, double Tau_2, double g_peak) const
...
void olm_receptors::handle(nest::SpikeEvent &e){
...
void olm_receptors::handle(nest::CurrentEvent& e){
  assert(e.get_delay_steps() > 0);
